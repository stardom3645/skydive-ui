...
export const KubernetesMetadataModal = ({
    ...
    return <HistoryModal
        ...
        <KubernetesModalSection
            title={`${kindLabel} ${entries.length}개`}
            description={description || `이 자원에 설정된 ${kindLabel} 키와 값입니다.`}>
...
export const KubernetesMetadataRows = ({ items, labelWidth = 122, emptyText = '추가 메타데이터 없음' }: KubernetesMetadataRowsProps) => {
    ...
    const visibleItems = normalized.filter(item => item.entries.length > 0)
    ...
    return <div className="netdive-detail-metadata-rows">
        {visibleItems.length === 0 && <DetailEmpty description={emptyText} compact />}
