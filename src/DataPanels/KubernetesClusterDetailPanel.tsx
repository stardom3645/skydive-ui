                    ...
                    groups={[
                        {
                            ...
                            items: [
                            { key: 'nodes', label: this.resourceLabel('node'), count: nodeSummary.total, icon: this.resourceIcon(nodeResource), iconTone: 'kubernetes', resources: connectedResourcePopoverItems(nodeResource.nodes, { anchorNodeID: this.props.node.id, nodeAttrs: this.props.nodeAttrs }), onClick: nodeResource.nodes.length ? () => this.focusResources(nodeResource.nodes) : undefined, tooltip: translate('kubernetesFocusNodes') },
                            { key: 'namespaces', label: this.resourceLabel('namespace'), count: namespaceCount, icon: this.resourceIcon(namespaceResource), iconTone: 'kubernetes', resources: connectedResourcePopoverItems(namespaceResource.nodes, { anchorNodeID: this.props.node.id, nodeAttrs: this.props.nodeAttrs }), onClick: namespaceResource.nodes.length ? () => this.focusResources(namespaceResource.nodes) : undefined, tooltip: translate('kubernetesFocusNamespaces') },
                            ...(workloadNodes.length ? [{ key: 'workloads', label: translate('kubernetesTopologyWorkloadControllers'), count: workloadNodes.length, icon: <DetailLayerIcon glyph={'\uf5fd'} />, iconTone: 'kubernetes' as const, resources: connectedResourcePopoverItems(workloadNodes, { anchorNodeID: this.props.node.id, nodeAttrs: this.props.nodeAttrs }), onClick: () => this.focusResources(workloadNodes), tooltip: translate('kubernetesTopologyWorkloadControllers') }] : []),
                            { key: 'pods', label: this.resourceLabel('pod'), count: displayedPodTotal, icon: this.resourceIcon(podResource), iconTone: 'kubernetes', resources: connectedResourcePopoverItems(activePodResources, { anchorNodeID: this.props.node.id, nodeAttrs: this.props.nodeAttrs }), onClick: activePodResources.length ? () => this.focusResources(activePodResources) : undefined, tooltip: translate('kubernetesFocusPods') },
                            { key: 'services', label: this.resourceLabel('service'), count: serviceCount, icon: this.resourceIcon(serviceResource), iconTone: 'kubernetes', resources: connectedResourcePopoverItems(serviceResource.nodes, { anchorNodeID: this.props.node.id, nodeAttrs: this.props.nodeAttrs }), onClick: serviceResource.nodes.length ? () => this.setState({ activeDetailTab: 'services', serviceNamespaceFilter: 'all' }) : undefined, tooltip: translate('kubernetesFocusServices') }
                            ]
                        },
                        {
                            key: 'storage',
                            ...
                            items: [
                                ...(persistentVolumeClaimResource.nodes.length ? [{ key: 'pvcs', label: this.resourceLabel('persistentvolumeclaim'), count: persistentVolumeClaimResource.nodes.length, icon: this.storageResourceIcon('persistentvolumeclaim'), iconTone: 'kubernetes' as const, resources: connectedResourcePopoverItems(persistentVolumeClaimResource.nodes, { anchorNodeID: this.props.node.id, nodeAttrs: this.props.nodeAttrs }), onClick: () => this.focusResources(persistentVolumeClaimResource.nodes), tooltip: 'PersistentVolumeClaim (PVC)' }] : []),
                                ...(persistentVolumeResource.nodes.length ? [{ key: 'pvs', label: this.resourceLabel('persistentvolume'), count: persistentVolumeResource.nodes.length, icon: this.storageResourceIcon('persistentvolume'), iconTone: 'kubernetes' as const, resources: connectedResourcePopoverItems(persistentVolumeResource.nodes, { anchorNodeID: this.props.node.id, nodeAttrs: this.props.nodeAttrs }), onClick: () => this.focusResources(persistentVolumeResource.nodes), tooltip: 'PersistentVolume (PV)' }] : []),
                                ...(storageClassResource.nodes.length ? [{ key: 'storage-classes', label: this.resourceLabel('storageclass'), count: storageClassResource.nodes.length, icon: this.storageResourceIcon('storageclass'), iconTone: 'kubernetes' as const, resources: connectedResourcePopoverItems(storageClassResource.nodes, { anchorNodeID: this.props.node.id, nodeAttrs: this.props.nodeAttrs }), onClick: () => this.focusResources(storageClassResource.nodes), tooltip: KUBERNETES_DETAIL_LABELS.storageClass }] : [])
                            ]
                        }
                    ]} />
