...
describe('Kubernetes detail UI contract', () => {
    ...
    it('keeps node information hierarchy and workload exploration compact', () => {
        ...
        assert.ok(node.includes('복제본이 하나뿐이어서 장애 시 서비스 중단 가능성이 있는 워크로드입니다.'))
    ...
    it('keeps risk evaluation rows separate from connected-resource exploration', () => {
        ...
        assert.ok(node.includes('const localStorageRiskData = localStorageRiskWorkloads.length ? localStorageRiskWorkloads : localStorageRiskPods'))
        assert.ok(node.includes("pending: { title: 'Pending Pod'"))
        assert.ok(node.includes("restart: { title: '재시작 이력 Pod'"))
        assert.ok(node.includes("'oom-killed': { title: '현재 OOMKilled Pod'"))
    ...
    it('uses the shared operational tooltip hierarchy for cluster and node KPIs', () => {
        ...
        assert.ok(node.includes('현재 노드에 배치되어 있고 종료 처리 중이 아닌 Pod 수입니다.'))
        assert.ok(node.includes('현재 노드의 활성 Pod 중 비정상 상태로 판정된 Pod 수입니다.'))
    ...
    it('keeps risk evidence rows on one shared three-column status pattern', () => {
        ...
        assert.ok(cluster.includes('<StatusEvidenceList>'))
        assert.ok(node.includes('<StatusEvidenceList>'))
        assert.ok(cluster.includes('state={<DetailStatusIndicator tone={tone}>{label}</DetailStatusIndicator>}'))
        ...
        assert.ok(node.includes('<DetailStatusIndicator tone="warning">보완 권장</DetailStatusIndicator>'))
        assert.ok(node.includes("<DetailStatusIndicator tone={tone === 'danger' ? 'danger' : 'warning'}>{tone === 'danger' ? '위험' : '보완 권장'}</DetailStatusIndicator>"))
    ...
    it('keeps the namespace panel on the cluster and node shared detail grammar', () => {
        ...
        assert.ok(namespace.includes("podModal: 'problem'"))
